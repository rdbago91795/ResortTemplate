-- 0024_write_booking.sql — T043, T044
--
-- ══════════════════════════════════════════════════════════════════════════════════════
-- THE SINGLE WRITER. Every state transition and every stay change goes through here.
-- ══════════════════════════════════════════════════════════════════════════════════════
--
-- There are three ways a booking gets written — a guest's hold (C2), an owner-created
-- booking (C4), an owner moving one (C6) — and a fourth writer claims occupancy: an owner
-- moving a block (FR-037a).
--
-- The exclusion constraint on room_occupancy protects OVERLAP for all four unconditionally,
-- because it is a constraint. The STATE MACHINE has no equivalent backstop: `with check`
-- constrains the new row but cannot see the old one, so it cannot express "confirmed may go
-- to cancelled but not back to held". Expressed as table policies it would be written three
-- times and load-tested once. Hence: one function, thin callers, and NO client write policy
-- on bookings at all.
--
-- WHAT THIS FUNCTION DERIVES AND NEVER ACCEPTS (constitution VIII f):
--   origin           from the caller — anon means online, admin means owner (FR-021f)
--   stay_total       from price_stay() (FR-003)
--   hold_expires_at  from site_settings.hold_minutes
--   room_unit_id     server-side selection; the caller names a TYPE
--   guests validity  against the stored room_types.max_occupancy, not a client value
--
-- ⚠ EVERY IDENTIFIER IS SCHEMA-QUALIFIED, INCLUDING TYPE NAMES. With `search_path = ''` a
-- bare `::citext` resolves to nothing, because citext lives in `public`. This was found by
-- the first end-to-end probe, not by review — qualifying tables and functions while leaving a
-- type cast bare is an easy miss.

create or replace function public.write_booking(
  p_action  text,
  p_payload jsonb
)
returns jsonb
...
begin
  ...
  if p_action = 'hold' then
    ...
    -- Re-checking later would take a guest's payment and then refuse the room.
    ...
    loop
      begin
        insert into public.bookings (
          ...
          p_payload ->> 'guest_name', (p_payload ->> 'guest_email')::public.citext,
        ...
        values (v_unit.id, 'booking', v_new_id,
  ...
  -- ═══ SUBMIT REFERENCE — anonymous guest ════════════════════════════════════
    ...
    where booking_reference = upper(trim(p_payload ->> 'booking_reference'))
    for update;
...
revoke all on function public.write_booking(text, jsonb) from public;
grant execute on function public.write_booking(text, jsonb) to anon, authenticated;

comment on function public.write_booking(text, jsonb) is
  'The only path that mutates bookings or room_occupancy. Owns every state transition (research R1). origin, stay_total, hold_expires_at, and unit selection are all derived here and never accepted from the caller.';
